...
- library-api-explainer: Explain library APIs comprehensively for production readiness. Break down into core concepts, surface map, flows, state management, error models, and operational best practices. Use when a user asks to explain a library, specific APIs, or when evaluating tutorial code. (file: `.agents/skills/library-api-explainer/SKILL.md`)
