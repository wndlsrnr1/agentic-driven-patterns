...
test("buildRuntimeConfigFromEnv throws when API_KEY is missing", (): void => {
  assert.throws(
...
test("runInformationProcessingWorkflow returns runner result", async (): Promise<void> => {
  const result: RunInformationProcessingWorkflowResult =
    await runInformationProcessingWorkflow({
      ...
      workflowRunner:
        async (): Promise<RunInformationProcessingWorkflowResult> => {
          return {
            sourceUrl: "https://example.com",
            ...
            knowledgeBaseFindings: [
              {
                ...
                sourceUrl: "https://kb.local/seoul",
              },
            ],
            finalReport: "report",
            modelName: "test-model",
