...
```bash
node --env-file=.env.modules --experimental-strip-types ./02-routing-patterns/coordinator-routing.workflow.ts
```

Coordinator single request mode:

```bash
node --env-file=.env.modules --experimental-strip-types ./02-routing-patterns/coordinator-routing.workflow.ts -- "Book me a flight to London."
```

`rout2.ts` single request workflow:

```bash
node --env-file=.env.modules --experimental-strip-types ./02-routing-patterns/rout2.ts -- "Book me a hotel in Paris."
```

`coordinator-routing-openai.ts` tutorial workflow (OpenAI Agents SDK style):

```bash
node --env-file=.env.modules --experimental-strip-types ./02-routing-patterns/coordinator-routing-openai.ts
```
